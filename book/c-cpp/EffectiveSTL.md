...
- 作为一个 `STL` 容器，`vector<bool>` 只有两点不对：
  ...
  - 其次，它并不存储 `bool`；
- 因为：
  - `vector<bool>` 为了节省空间，它存储的是 `bool` 的紧凑表示。在一个典型的实现中，存储在 `vector` 中的每个 `bool` 仅占一个二进制位，因此它实际存储的并不是 `bool` 对象；
  - 能够寻址的最小单位为字节，而位域无法寻址，导致无法通过指针或引用指涉到一个位域值；
  - 一个对象要成为 `STL` 容器，需要满足的一个条件是：如果 `c` 是包含对象 `T` 的容器，而且 `c` 支持 `operator[]`，那么下面的代码必须能够通过编译：
    ```cpp
    T *p = &c[0];
    ```
    然而，正如前一点提高的，`&c[0]` 是错误的，无法通过编译。
- 代替措施：
  - 使用 `deque<bool>`：是一个 `STL` 容器，而且它确实存储 `bool`；但 `deque` 的内存并不是连续，无法将其传递给一个期望 `bool` 数组的 `C API`（`vector<bool>` 也不能这么做）；
  - 使用 `bitset`，它不是 `STL` 容器，要求**其大小在编译时就确定**；不支持插入和删除元素；同样也使用了一种紧凑表示，只为所包含的每个值提供一位空间；**如果不需要迭代器和动态地改变大小，可以使用 `bitset`**；
